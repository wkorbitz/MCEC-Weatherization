...
params.i = 3000; % say run at 3000 A/m^2
...
params.E_el_act = 1150; % K
params.E_ion_act = 9923.07; % K
params.A_el = 9.50e7; % [S-K/m]
params.A_ion = 216.43e2; % S/m

params.sigma_el = params.A_el/params.T * exp(-params.E_el_act/params.T);
params.sigma_ion = params.A_ion/params.T * exp(-params.E_ion_act/params.T);
params.sigma_el_eff = params.sigma_el * 1.5^(1.5);
params.sigma_ion_eff = params.sigma_ion * 1.5^(1.5);

params.i_0 = 23.8; % DUMMY VALUE FOR i_0
params.beta = 0.5;
params.n_t = 2; % 2e^- reactions

% TPB parameters
%% Chen et al., for binary mixtures only (focus on page 242)

r_ed = 240e-6; % [m] Ni particle radius
r_el = 10e-6; % [m] YSZ particle radius

theta = deg2rad(15); % [degree] Contact angle, from pg. F1252 of Rahmanipour et al.
r_c = min(r_ed,r_el)/2*sin(theta); % [m] Neck radius
l_ed_el = 2*pi*r_c; % [m] Contact perimeter

Z_ed_ed = 1.764; % Coordination number between electrodes
Z_el_el = Z_ed_ed; % Same as above but for electrolytes



P_el = (1-((3.764-Z_el_el)/2)^2.5)^0.4; % Probability that electrolyte particles belong to an A-cluster

P_ed = 1-P_el;  % Probability of above, but of electrodes
%r_g = 2*phi_g/A_s_g_V;

psi_ed = 0.5; % Look at page 242, after eq (5)
psi_el = psi_ed;

xi_ed = (psi_ed/r_ed^3)/(psi_el/r_el^3);
xi_el = (psi_el/r_el^3)/(psi_ed/r_ed^3);

n_ed_V = (1-params.epsilon)*psi_ed/(4*pi*r_ed^3/3);  % Number of electrode particles

% Z_bar = xi_ed*Z_ed + xi_el*Z_el;  % see eq (6)
% Z_bar = xi_ed*Z_ed_ed + xi_el*Z_el_el + 2*xi_ed*Z_ed_el; Circular
% definition issue
Z_bar = 6; % see information before eq (22)

Z_ed_el = 0.5*(1+r_ed^2/r_el^2)*Z_bar*(psi_el/r_el)/(psi_ed/r_ed);

params.lambda_TPB_V = l_ed_el*n_ed_V*Z_ed_el;
...
function [F_of_C, r_WGS, r_SMR, eta_act] = compute_F(C, params)
    [Ns, Nz] = size(C);
    z = params.z;
    %J = zeros(Ns, Nz);
    ...
    [phi_el, phi_ion] = compute_phi(params);
    lambda_TPB_V = params.lambda_TPB_V;
    eta_act = phi_el - phi_ion;
    S_el = params.nu_el * i_0 * lambda_TPB_V/(4*params.F) * ( exp((beta * n_t * params.F * eta_act)/(params.R * params.T)) ...
        - exp(((1-beta) * n_t * params.F * eta_act)/(params.R * params.T)) );
...
function [phi_el, phi_ion] = compute_phi(params)
    % Insert two coupled Ohm's Law-BV PDE
    % The idea is to implement Newton's method to tackle its nonlinear
    % nature
    tol = 1e-10;
    z = params.z;
    Nz = params.Nz;
    i_curr = params.i;
    sigma_el_eff = params.sigma_el_eff;
    sigma_ion_eff = params.sigma_ion_eff;

    U = ones(1, 2*Nz);
    F = zeros(size(U));
    %% Handle BCs %%
    E_N = 1.3; % [V] for instance
    h_1 = z(2) - z(1);
    hN = z(end)-z(end-1);
    right_bc_curr = -2 * i_curr / (hN); % current supplied externally
    lambda_TPB_V = params.lambda_TPB_V;
    beta = params.beta;
    n_t = params.n_t;
    i_0 = params.i_0;
    alpha_BV = beta*n_t*params.F/(params.R*params.T);
    gamma_BV = (1-beta)*n_t*params.F/(params.R*params.T);

    % Left BC stencil coefficients
    p = 2/h_1^2;
    q = p;

    % Right BC stencil coefficients
    j = 2/hN^2;
    k = j;
    ...
    delta_U = ones(1, 2*Nz);
    while norm(delta_U)/norm(U) < tol
        % Enforce left BC
        F(1) = U(1) - E_N;
        F(2) = sigma_ion_eff * (p*U(2) + q*U(4));
        % Enforce right BC
        F(end-2) = sigma_el_eff * (j*U(end-3) + k*U(end-1));
        F(end-1) = sigma_ion_eff * (r*U(end-2) + s*U(end)) - right_bc_curr;
    ...
    %% Internal nodes %%
        for i = 2:Nz-1
            ...
            delta_phi = U(col_el_i) - U(col_ion_i);
            BVterm = exp(alpha_BV * delta_phi) - exp(gamma_BV * delta_phi);
            S = -i_0 * lambda_TPB_V * BVterm;

            F(k_el) = sigma_el_eff * (a*U(col_el_im1) + b*U(col_el_i) + c*U(col_el_ip1) - S);
            F(k_ion) = -sigma_ion_eff * (a*U(col_ion_im1) + b*U(col_ion_i) + c*U(col_ion_ip1) - S);

            %% Jacobian hon hon hon %%
            S_prime_ion = i_0 * lambda_TPB_V * (alpha_BV * exp(alpha_BV * delta_phi) + gamma_BV * exp(gamma_BV * delta_phi)  );
            S_prime_el = -S_prime_ion;

            d_m2(k_el) = d_m2(k_el) + sigma_el_eff * a;
            d_m2(k_ion) = d_m2(k_ion) - sigma_ion_eff * a;
            d_m1(k_ion) = d_m1(k_ion) - S_prime_ion;
            d_0(k_el) = d_0(k_el) + sigma_el_eff * b + S_prime_el;
            d_0(k_ion) = d_0(k_ion) - sigma_ion_eff * b + S_prime_ion;
            d_p1(k_el) = d_p1(k_el) - S_prime_el;
            d_p2(k_el) = d_p2(k_el) + sigma_el_eff * c;
            d_p2(k_ion) = d_p2(k_ion) - sigma_ion_eff * c;

            J = spdiags([d_m2 d_m1 d_0 d_p1 d_p2], [-2, -1, 0, 1, 2], 2*Nz, 2*Nz);
...
function F = rhs_sofc(t, y, params)
    ...
    [F_matrix, r_WGS, r_SMR, eta_act] = compute_F(C, params);
...
[~, eta_act, r_WGS_final, r_SMR_final] = compute_F(C_final, params);
